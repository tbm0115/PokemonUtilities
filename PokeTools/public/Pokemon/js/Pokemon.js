...
const dbPromise = idb.open('user-dex', 1, upgradeDB => {
  var games, pokemon;
...
var Game = (function (db) {
  ...
  this.Add = (function (nid) {
    ...
    if (this.pokemon.filter(function (e, i) { return e.nid === nid; }).length <= 0) {
      console.log("Adding Pokemon '" + nid + "'!");
      // Create Pokedex Entry
      var pde = new PokedexEntry({
        nid: nid,
        game: $this.name,
        callback: function (pde) {
...
var UserDex = {
  Games: {
    Items: new Array(),
    Add: function (name) {
      if (UserDex.Games.Items.filter(function (e, i) { return e.name.toLowerCase() === name.toLowerCase(); }).length <= 0) {
        var nwGame = new Game({
          name: name
        });
        // Update IndexedDb
